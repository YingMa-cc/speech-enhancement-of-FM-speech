...
class STFT(torch.nn.Module):
    def __init__(self, filter_length=1024, hop_length=512):
        ...
        inv_fourier_basis = inv_fourier_basis / (window+self.EPSILON)
        cutoff = int((self.filter_length / 2 + 1))
    ...
    def spec_transform(self, input_data, is_log=False):
        real = input_data[:, :, :, 0]
        imag = input_data[:, :, :, 1]
        output = torch.sqrt(real ** 2 + imag ** 2)
        if is_log:
            output = torch.log(output + self.EPSILON)
        return output