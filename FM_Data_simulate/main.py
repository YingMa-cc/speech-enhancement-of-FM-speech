...
def wgn_add(clean, noise, snr):
    ps = np.sum(abs(clean) ** 2) / len(clean)
    pn = ps / (10 ** (snr / 10))
    noise = noise * np.sqrt(pn)
    signal_add_noise = clean + noise
    return signal_add_noise
...
for z in range(1000):
    for i in range(len(files)):
        ...
        for j in range(N):
            ...
            # 根据时延向后移动信号，开头补0,时域多径效应
            # f = np.concatenate([np.zeros(int(tau[j])), f2.real[0:(len(f2)-int(tau[j]))]])
            # 每条路径信号叠加
            if j == 0:
